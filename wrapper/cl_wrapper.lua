...
function Wrapper:RemoveMoney(type,amount)
    TriggerServerEvent("Wrapper:skydive:RemoveMoney",type,amount)
end
...
function Wrapper:Stash(label,weight,slots) -- Create and Open a stash in qb-inventory
    ...
    if Config.Settings.Inventory == "OX" then 
        TriggerServerEvent("Wrapper:skydive:Inventory:Stash:Ox",label)
        Wait(500)
        TriggerServerEvent("Wrapper:skydive:Inventory:Stash:Ox:Open",label)
    end
    if Config.Settings.Inventory == "QS" then 
        TriggerServerEvent("Wrapper:skydive:Inventory:Stash:QS",label)
        Wait(500)
        local other = {}
...
function Wrapper:Bill(playerId, amount) -- QBCore bill player, YOU (your job) Bills => Player and amount (player,amount)
    TriggerServerEvent("Wrapper:skydive:Bill",playerId, amount)
end
...
function Wrapper:RemoveItem(item,amount)
    ...
    TriggerServerEvent("Wrapper:skydive:RemoveItem", item, amount)
end

function Wrapper:AddMoney(type,amount) -- AddItem to me (Like give item) very unsafe use only in dev build.
    TriggerServerEvent("Wrapper:skydive:AddMoney",type,amount)
end
...
function Wrapper:Log(txt) -- Log all of your abusive staff
    TriggerServerEvent("Wrapper:skydive:Log",txt)
end
...
function Wrapper:Tp(_coords,fancy,ped) -- Teleport to coords, very fancy, very pretty
    ...
    if coords == nil then 
        QBCore.Functions.Notify("Wrapper:skydive: Нямаш coords бай хуй", "error", 2500)
        return
    end
    if fancy then 
